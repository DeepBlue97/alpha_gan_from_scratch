...
class alpha_Trainer(object):
    def __init__(self, data_loader, config, devices=None):
    ...
    def train(self):
        ...
        # if self.use_tensorboard:
        #     self.writer.add_image('img/fixed_img', denorm(fixed_img[0].data), 0)
        # else:
        ...
        for step in range(start, self.total_step):
            ...
            # g_loss.detach_().requires_grad_(True)
            ...
            # d_loss.detach_().requires_grad_(True)
            ...
            # c_loss.detach_().requires_grad_(True)
            c_loss.backward()
            ...
            if (step + 1) % self.sample_step == 0:
                ...
                if self.use_tensorboard:
                    self.writer.add_image('img/rec_from_z', denorm(rec_from_z[0].data), step + 1)
                    self.writer.add_image('img/img_from_z', denorm(img_from_z[0].data), step + 1)
                else:
                    save_image(denorm(img_from_z[0].data),
                            os.path.join(self.sample_path, '{}_img_from_z.png'.format(step + 1)))
                    save_image(denorm(rec_from_z[0].data),
    ...
    def build_model(self):
        ...
        if self.parallel:
            self.G = nn.DataParallel(self.G, device_ids=[0,1,2,3,4,5,6,7])
            self.E = nn.DataParallel(self.E, device_ids=[0,1,2,3,4,5,6,7])
            self.D = nn.DataParallel(self.D, device_ids=[0,1,2,3,4,5,6,7])
            self.C = nn.DataParallel(self.C, device_ids=[0,1,2,3,4,5,6,7])
